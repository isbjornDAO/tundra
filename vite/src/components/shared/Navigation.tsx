...
const Navigation = () => {
    ...
    //const { adminData } = useUserContext();
    ...
    const isTournamentPath = tournamentItems.some(item => location.pathname === item.href);
                    ...
                    <div className="absolute top-full left-0 mt-2 w-48 bg-black/95 backdrop-blur-sm border border-white/20 rounded-lg shadow-xl py-2" style={{ zIndex: 999999 }}>
                        {tournamentItems.map((item) => (
                            <Link
                                ...
                                className={`block w-full px-4 py-3 text-left transition-all duration-200 flex items-center gap-3 ${location.pathname === item.href
