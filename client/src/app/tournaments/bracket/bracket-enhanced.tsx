                      ...
                      <div key={round} className="bg-white/5 rounded-xl p-6 border border-white/10">
                        ...
                        <div className="space-y-6">
                          {roundMatches.map((match) => (
                            <EnhancedMatchCard key={match._id} match={match} userAddress={address} />
                          ))}
                        </div>
                      </div>
                    );
...
function EnhancedMatchCard({ match, userAddress }: { match: Match; userAddress: string | undefined }) {
  const isUserMatch = (match.clan1?.leader === userAddress || match.clan2?.leader === userAddress);
  
  return (
    <div className={`border rounded-xl p-6 transition-all ${
      match.status === 'completed' ? 'bg-gradient-to-br from-green-500/10 to-emerald-500/10 border-green-500/30' :
